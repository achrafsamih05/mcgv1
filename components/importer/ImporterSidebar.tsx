...
export function ImporterSidebar({
  ...
  active: ImporterSectionId;
  onSelect: (id: ImporterSectionId) => void;
  fullName: string;
  companyName?: string;
  unread: number;
  mobileOpen: boolean;
  ...
  return (
    <>
      ...
      <aside
        ...
        aria-label="Importer navigation"
      >
        <div className="flex items-center justify-between border-b border-white/10 px-5 py-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-sm font-bold text-white">MCG <span className="text-accent-500">Importer</span></p>
              <p className="text-[11px] uppercase tracking-wider text-navy-400">Buyer Console</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="border-t border-white/10 px-5 py-4">
          ...
          {companyName && <p className="truncate text-xs text-navy-400">{companyName}</p>}
        </div>
      </aside>
    </>
  );
