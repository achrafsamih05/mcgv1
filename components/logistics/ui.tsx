...
import type { Availability, VerificationState } from "@/lib/logistics/types";
...
export function VerificationBadge({
  ...
  const label = kind === "Transport Company" ? "Verified Transport Company" : "Verified Driver";
  if (state === "Approved") {
    return (
...
export function AvailabilityBadge({ status }: { status: Availability }) {
  const map: Record<Availability, { tone: Tone; dot: string }> = {
    "Available Now": { tone: "success", dot: "bg-emerald-500" },
    Busy: { tone: "accent", dot: "bg-accent-500" },
    "Out of Service": { tone: "danger", dot: "bg-red-500" },
  };
